...
def request(query, params):
    ...
    domain = EBAY_DOMAINS.get(region, EBAY_DOMAINS.get(lang, "ebay.com"))
    offset = (params.get("pageno", 1) - 1) * 10
    
    price_term = "precio" if lang in ("es", "pt") else "price"
    
    query_params = {
        "q": f"site:{domain}/itm {query} {price_term}",
        "hl": lang,
        "num": 10,
        "start": offset,
        "pws": "0"
...
async def response(resp):
    ...
    for node in nodes:
        ...
        if title_node and url_node:
            ...
            if url and url.startswith("http") and ("ebay." in url and "/itm/" in url):
                title = title_node.text().strip()
                content = snippet_node.text().strip() if snippet_node else "Ver producto en eBay."
                
                # Clean eBay titles
                for clean_pref in ["eBay: ", "eBay.es: ", "eBay.com: "]:
    ...
    if not results or is_blocked:
        try:
            ...
            if not original_q:
                original_q = f"site:ebay.com/itm {clean_query} precio"
            
            lang = resp.search_params.get("language", "es")
            fallback_results = await fetch_fallback_search(original_q, lang, resp.client)
            for r in fallback_results:
                url_lite = r["url"]
                if url_lite and url_lite.startswith("http") and ("ebay." in url_lite and "/itm/" in url_lite):
                    title_lite = r["title"]
                    for clean_pref in ["eBay: ", "eBay.es: ", "eBay.com: "]:
                    ...
                    res = {
                        "title": title_lite,
                        "url": url_lite,
                        "content": r["content"] or "Ver producto en eBay.",
                        "source": "ebay",
                        "template": "shopping.html"
                    }
                    if price_lite:
            ...
            print(f"EXCEPTION in ebay fallback: {e}")
    ...
    if results:
        try:
            ...
            print(f"Exception enriching eBay results: {e}")
