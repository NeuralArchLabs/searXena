...
# Windows compatibility: pwd module doesn't exist on Windows
if sys.platform == 'win32':
    import getpass
    class PwdUser:
        def __init__(self):
            self.pw_name = getpass.getuser()
            self.pw_uid = os.getuid() if hasattr(os, 'getuid') else 0
    def getpwuid(uid):
        return PwdUser()
    pwd = type('pwd', (), {'getpwuid': getpwuid})()
else:
    import pwd
...
def initialize():
    ...
    try:
        ...
        _CLIENT = None
        _pw = pwd.getpwuid(os.getuid() if hasattr(os, 'getuid') else 0)
