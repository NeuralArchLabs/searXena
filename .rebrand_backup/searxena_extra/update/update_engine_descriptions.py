...
from searx.utils import extract_text, searxena_useragent
...
def get_wikipedia_summary(wikipedia_url, searxena_locale):
    # get the REST API URL from the HTML URL

    # Headers
    headers = {'User-Agent': searxena_useragent()}

    if searxena_locale in WIKIPEDIA_LANGUAGE_VARIANTS:
        headers['Accept-Language'] = WIKIPEDIA_LANGUAGE_VARIANTS.get(searxena_locale)
    ...
    if result is not None:
        for binding in result['results']['bindings']:
            ...
            for engine_name in wd_to_engine_name[wikidata_id]:
                for searxena_locale in LOCALE_NAMES:
                    if WIKIPEDIA_LANGUAGES[searxena_locale] != wikidata_lang:
                        continue
                    print(
                        f"    engine: {engine_name:20} / wikidata_lang: {wikidata_lang:5}",
                        f"/ len(wikidata_desc): {len(desc)}",
                    )
                    update_description(engine_name, searxena_locale, desc, 'wikidata')
        ...
        for binding in result['results']['bindings']:
            ...
            for engine_name in wd_to_engine_name[wikidata_id]:
                for searxena_locale in LOCALE_NAMES:
                    if WIKIPEDIA_LANGUAGES[searxena_locale] != wikidata_lang:
                        continue
                    desc = get_wikipedia_summary(wikipedia_url, searxena_locale)
                    if not desc:
                        continue
                    print(
                        f"    engine: {engine_name:20} / wikidata_lang: {wikidata_lang:5}",
                    ...
                    update_description(engine_name, searxena_locale, desc, 'wikipedia')
