...
| `category`        | `string`   | No        | `general` (default), `news`, `shopping`, `images`, `videos`, `maps`. |
...
2.  **Categorical Precision:** Use `category: "general"` for technical or broad topics and `category: "news"` for recent events.
...
results = search_locally("Agentic workflows best practices", category="general")
