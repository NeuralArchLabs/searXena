...
searXena 将您的数据**绝对不对外泄露**（流向第三方广告商的配置文件中）作为最高优先级，通过在图形界面之下建立屏蔽层来实现。尽管如此，我们的架构也基于几项技术共识，在此公开披露：
...
*   **信息来源:** searXena 作为一个信号聚合器运行。我们认可并尊重集成搜索引擎（Google, Bing, DuckDuckGo 等）所提供的巨大索引工作和技术价值。本软件仅限于为最终用户处理和匿名化公共数据。
*   **教育与研究用途:** searXena 仅供研究和个人使用。开发人员不鼓励也不对使用本工具违反第三方服务条款的行为负责。
