...
*   **信息来源:** searXena 作为一个公开信号聚合器运行。我们认可并尊重集成搜索引擎（Google, Bing, DuckDuckGo 等）所提供的巨大索引工作和技术价值。本软件作为可视化及匿名化公共数据的工具提供给最终用户。
*   **教育与研究用途:** searXena 仅供研究和个人使用。开发人员不鼓励也不对使用本工具违反第三方服务条款的行为负责。我们提供中立的搜索体验；如果您希望获得由顶级跟踪算法带来的便捷与个性化体验，我们强烈建议您直接使用那些具有声望且技术底蕴深厚的专业平台。
