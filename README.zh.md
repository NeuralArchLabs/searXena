...
- **抓取与提取:** `O-ZEN Engine`（高性能阅读模式提取核心，基于 Trafilatura）。
- **DOM 解析器:** `lxml` 与 `selectolax` 以及模块化选择器结合用于深度 DOM 解析。
...
* 🧘 **O-ZEN Engine (阅读模式):** 内置工业级提取引擎，支持在无广告、无干扰、无追踪脚本的环境下阅读文章、博客和百科内容。
...
*   **信息来源:** searXena 作为一个公开信号聚合器运行。我们尊重并认可各外部搜索引擎（Google, Bing, DuckDuckGo 等）在索引领域所做的工作。searXena 通过查询其公开数据源来整合结果，作为独立的数据可视化与匿名化接口运行，与上述服务商无任何关联、背书或商业合作关系。
