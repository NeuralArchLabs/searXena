<div align="center">
  ...
  <p>
    一个敏捷、本地化且 100% 原生的 Windows 元搜索引擎，专注于极端隐私。
  </p>
...
* 🛡️ **绝对隐私保护盾:** 作为您和大型科技公司之间的中介代理。无论是 Google 还是 Microsoft 都无法对您进行画像；所有请求从他们那边看来均源自 searXena 服务。
...
* 由 **Armando Martínez Palomera** 独立开发并重构。
* 其中关于数据提取模块定义及规避爬虫分析引擎启发模式的核心底层技术知识，其全部道德归属均属于伟大并令人敬畏的 [Searx](https://github.com/searx/searx) (Taica发起) 及 [SearXNG](https://github.com/searxng/searxng) 全部过去与现役代码维护人员。
* 只有仰赖如（包含维基媒体基金会等成员在内的）实体组织的包容性协议原则，我们依赖社区爬虫构建此类项目的能力才成为可能。

*searXena 在此以“如是”状态直接进行散布并遵循“使用后风险自负”理念。我们不会贮存访问日志，无法控制您的检索倾向，更不会对例如“由于您的搜寻记录导致宽带服务商惊吓”或是“惹怒某某超大型商业企业导致您被控逃避跟踪防线”等责任承担后果。明智地运用您的利器，并永葆互联网自由无垠！*
