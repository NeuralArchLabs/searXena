...
* 📱 **现代动态 UI/UX:** 流畅的动画、极致的深色模式（"太空紫"）、响应式界面，并按类别分页呈现（综合、新闻、购物、地图、视频、图片）。
...
### 绝对 DOM 代理化
当您查询综合类内容（新闻、编程或其他公开主题）时，searXena 的后台引擎会保护您的身份。我们系统性地调整 `User-Agent`，从商业引擎拉取到的图像网址均经过内部 `/proxify` 代理路由系统处理，从而确保您的本地 IP 不会被直接暴露给第三方服务器。
...
* **抗幻觉的智能排序引擎:** 启发式算法始终站在 Agent 一侧处理搜索回报，向 LLM 隐藏内容营销和低质量分发网络，优先输送来自官方文档、百科来源以及核心开源库的可靠摘要。
